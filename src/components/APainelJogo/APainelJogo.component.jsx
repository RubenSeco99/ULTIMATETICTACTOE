...
function PainelJogo(props) {
    ...
    const [pausarContador_X, setPausarContador_X] = useState(false);
    const [pausarContador_O, setPausarContador_O] = useState(false);
    ...
    useEffect(() => {
        if (currentPlayer === "X") {
            setPausarContador_O(true);
            setPausarContador_X(false);
        } else {
            setPausarContador_O(false);
            setPausarContador_X(true);
        }
        return;
    }, [currentPlayer]); 
    ...
    const handleButtonClick = (index, currentPlayer) => {
        if(buttonValues[index] === null){
            ...
            nextTab = 1+(index % 9);             //é o numero do proximo tabuleiro de 1 a 9
            prepareNextTab(nextTab);
            // console.log(nextTab);
            // <Temporizador />
            if(currentPlayer === "X"){
                setPausarContador_X(false);
    ...
    return (
        ...
        <section id="dataFromForm">
            ...
            <div id="Temporizadores">
                <div className="Temporizador-X">
                    {currentPlayer === "X" ? <Temporizador jogador={currentPlayer} pausarContador_X={pausarContador_X} /> : <Temporizador jogador={null} />}
                </div>
                <div style={{ display: props.numberofplayers === true ? "block" : "none" }}className="Temporizador-O">
                    {currentPlayer === "O" ? <Temporizador jogador={currentPlayer} pausarContador_O={pausarContador_O} /> : <Temporizador jogador={null} />}
                </div>
            </div>
        </section>
...
/// AQUI PARA BAIXO É O CODIGO QUE ESTAVA ANTES DO AFONSO MEXER 
